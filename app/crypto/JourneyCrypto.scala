...
class JourneyCrypto @Inject() (crypto: Crypto) {
  ...
  private def mapPii(journey: Journey)(f: String => String): Journey = journey
    ...
    .modify(_.traceIndividualResponse.each.surname).using(f)
    .modify(_.traceIndividualResponse.each.addressLine1).using(f)
    .modify(_.traceIndividualResponse.each.addressLine2).using(f)
    .modify(_.traceIndividualResponse.each.addressPostcode.value).using(f)
