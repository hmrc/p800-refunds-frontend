...
class JourneyCrypto @Inject() (crypto: Crypto) {
  ...
  private def mapPii(journey: Journey)(f: String => String): Journey = journey
    ...
    .modify(_.bankAccountSummary.each.accountIdentification.each.value).using(f)
    .modify(_.bankAccountSummary.each.calculatedOwnerName.each.value).using(f)
    .modify(_.bankAccountSummary.each.accountOwnerName.each.value).using(f)
    .modify(_.bankAccountSummary.each.displayName.each.value).using(f)
    .modify(_.bankAccountSummary.each.parties.each.each.fullLegalName.each.value).using(f)
    .modify(_.bankAccountSummary.each.parties.each.each.name.each.value).using(f)
}
